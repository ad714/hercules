...
import { fetchPolyMarkets, searchPolyMarkets, filterPolyMarkets, type PolyMarket } from './services/poly';
import { computeMatchScore, extractTeams, type MatchResult } from './services/matcher';
...
export default function App() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const filteredFliq = filterFliqMatches(fliqRaw);
      ...
      filteredFliq.forEach(fq => {
        filteredPoly.forEach(pm => {
          ...
          if (score > 0.35) { // Balanced threshold
            matches.push({ fliq: fq, poly: pm, score });
          }
        });
      });

      // Deduplicate matches (keep highest score for each fliq question)
      const bestMatches = matches.reduce((acc, m) => {
      ...
      // DEEP SCAN: Only for markets NOT matched in the initial batch
      const unmatchedSports = filteredFliq
        .filter(fq => !bestMatches.some(m => m.fliq.questionId === fq.questionId))
        .filter(fq => {
          const header = (fq.blockchainMetadata.parentQuestionHeader || fq.blockchainMetadata.questionHeader || '').toLowerCase();
          return header.includes(' vs ') || header.includes(' against ') || header.includes(' at ');
        })
        .slice(0, 5); // Limit to 5 per sync to avoid rate limits

      if (unmatchedSports.length > 0) {
        console.log(`[Hercules] Deep scanning ${unmatchedSports.length} potential sports matches...`);
        for (const fq of unmatchedSports) {
          const [fa, fb] = extractTeams(fq.blockchainMetadata.parentQuestionHeader || fq.blockchainMetadata.questionHeader || '');
          const keywords = (fa && fb) ? `${fa} ${fb}` : (fq.blockchainMetadata.parentQuestionHeader || fq.blockchainMetadata.questionHeader).replace(/vs|against|at|beat|and/gi, ' ').replace(/\s+/g, ' ').trim();

          if (keywords) {
            console.log(`[Hercules] Searching Poly for: "${keywords}"`);
            const polySearchResults = await searchPolyMarkets(keywords);

            polySearchResults.forEach(pm => {
              const score = computeMatchScore(fq, pm);
              if (score > 0.35) {
                const fliqTitle = fq.blockchainMetadata.parentQuestionHeader || fq.blockchainMetadata.questionHeader;
                console.log(`[Hercules] ✅ Found Match! Score: ${score}`, { fliq: fliqTitle, poly: pm.title || pm.question });
                bestMatches.push({ fliq: fq, poly: pm, score });
              }
            });
          }
        }
      ...
      if (filteredFliq.length > 0 && !selectedMarket) {
        setSelectedMarket(filteredFliq[0]);
      }

      prefetchLiquidity(filteredFliq.slice(0, 30));
    } catch (e: any) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        padding: '0 20px', height: '60px', background: '#161b22', borderBottom: '1px solid #30363d',
        zIndex: 100
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
          <h1 style={{ fontSize: '1.4rem', fontWeight: 900, color: '#58a6ff', margin: 0, letterSpacing: '-0.5px' }}>HERCULES</h1>
          <nav style={{ display: 'flex', gap: '8px' }}>
            {[
              { id: 'arb', label: 'ARB TERMINAL', icon: '⚡' },
              ...
              <button
                ...
                style={{
                  background: currentView === tab.id ? '#1f6feb22' : 'transparent',
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        {currentView === 'arb' && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '24px', overflowY: 'auto' }}>
            <div style={{ marginBottom: '20px' }}>
              <h2 style={{ fontSize: '1.5rem', fontWeight: 700, margin: '0 0 8px 0' }}>Cross-Platform Discovery</h2>
              <p style={{ color: '#8b949e', fontSize: '0.9rem', margin: 0 }}>Real-time arbitrage opportunities between Fliq and Polymarket</p>
            </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                <thead>
                  <tr style={{ background: '#0d1117', borderBottom: '1px solid #30363d' }}>
                    <th style={{ padding: '16px', fontSize: '0.75rem', color: '#8b949e', textTransform: 'uppercase' }}>Market</th>
                    <th style={{ padding: '16px', fontSize: '0.75rem', color: '#8b949e', textTransform: 'uppercase' }}>Fliq Price</th>
                    <th style={{ padding: '16px', fontSize: '0.75rem', color: '#8b949e', textTransform: 'uppercase' }}>Poly Price</th>
                    <th style={{ padding: '16px', fontSize: '0.75rem', color: '#8b949e', textTransform: 'uppercase' }}>Match Score</th>
                    <th style={{ padding: '16px', fontSize: '0.75rem', color: '#8b949e', textTransform: 'uppercase' }}>Action</th>
                  </tr>
                </thead>
                <tbody>
                  {matchedMarkets.length > 0 ? matchedMarkets.map((match, i) => {
                    const priceArr = match.poly.outcomePrices || (match.poly as any).outcome_prices;
                    const polyPrice = (priceArr && priceArr[0]) ? priceArr[0] : '0.50';
                    return (
                      <tr key={i} style={{ borderBottom: '1px solid #21262d', transition: '0.1s' }}>
                        <td style={{ padding: '16px' }}>
                          <div style={{ fontWeight: 600, fontSize: '0.9rem', color: '#c9d1d9' }}>{match.fliq.blockchainMetadata.parentQuestionHeader}</div>
                          <div style={{ fontSize: '0.75rem', color: '#8b949e', marginTop: '4px' }}>{match.poly.title}</div>
                        </td>
                        <td style={{ padding: '16px' }}>
                          <div style={{ color: '#c9d1d9', fontWeight: 700 }}>$0.500</div>
                          <div style={{ fontSize: '0.65rem', color: '#8b949e' }}>Fliq LTP</div>
                        </td>
                        <td style={{ padding: '16px' }}>
                          <div style={{ color: '#58a6ff', fontWeight: 700 }}>${parseFloat(polyPrice).toFixed(3)}</div>
                          <div style={{ fontSize: '0.65rem', color: '#8b949e' }}>Poly Ask</div>
                        </td>
                        <td style={{ padding: '16px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            <div style={{ flex: 1, height: '4px', background: '#30363d', borderRadius: '2px', width: '60px' }}>
                              <div style={{ width: `${match.score * 100}%`, height: '100%', background: '#238636', borderRadius: '2px' }} />
                            </div>
                            <span style={{ fontSize: '0.75rem', fontWeight: 800, color: '#238636' }}>{(match.score * 100).toFixed(0)}%</span>
                          </div>
                        </td>
                        <td style={{ padding: '16px' }}>
                          <button
                            onClick={() => {
                              setSelectedMarket(match.fliq);
                              setCurrentView('all');
                            }}
                            style={{
                              padding: '6px 12px', background: '#1f6feb', color: '#fff',
                    ...
                    <tr>
                      <td colSpan={5} style={{ padding: '40px', textAlign: 'center', color: '#8b949e' }}>
                        {loading ? 'Finding cross-platform matches...' : 'No high-confidence matches found yet. Try syncing.'}
                      </td>
                    </tr>
                  )}
                </tbody>
        ...
        {/* VIEW: ALL MARKETS (Existing Dashboard) */}
        {currentView === 'all' && (
          <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
